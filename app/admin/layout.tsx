...
import { 
  ...
  PlusCircle, Wallet, Clock // Import icon Clock
} from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuNavigation = [
    ...
    // --- INPUT KASIR ---
    { name: "Kasir (Input)", href: "/admin/offline-orders", icon: <PlusCircle size={20} /> },
    
    // --- RIWAYAT KASIR (BARU) ---
    { name: "Riwayat Kasir", href: "/admin/offline-history", icon: <Clock size={20} /> },

    // --- PESANAN WEB (ONLINE) ---
    { name: "Pesanan Online", href: "/admin/online-orders", icon: <ShoppingBag size={20} /> },
    
    { name: "Laporan Keuangan", href: "/admin/finance", icon: <Wallet size={20} /> },
    ...
    { name: "3.AM Care (Donasi)", href: "/admin/donations", icon: <Heart size={20} /> },
  ];
      ...
      <div className="flex min-h-screen bg-[#F9F5E8] font-sans text-[#3E2723]">
        <aside className="w-72 bg-[#3E2723] text-[#F3EDE2] flex flex-col fixed h-full z-50 shadow-2xl">
          ...
          <div className="p-6 border-t border-white/5 space-y-3">
            <Link href="/admin/settings" className="w-full flex items-center gap-3 px-5 py-3 rounded-xl transition-all text-xs font-medium text-white/40 hover:text-white">
               <Settings size={16} /> Settings
            </Link>
            <button onClick={() => { localStorage.removeItem("isAdminLoggedIn"); router.push("/admin/login"); }} className="w-full flex items-center gap-3 px-5 py-4 bg-red-500/10 text-red-300 hover:bg-red-500 hover:text-white rounded-2xl transition-all text-sm font-bold">
        ...
        <div className="flex-1 ml-72 flex flex-col min-h-screen">
          <header className="h-24 sticky top-0 z-40 px-10 flex items-center justify-between bg-[#F9F5E8]/80 backdrop-blur-md">
            ...
            <div className="text-right">
                <p className="text-sm font-bold text-[#3E2723] opacity-80">Admin Area</p>
                <div className="flex items-center justify-end gap-1.5 mt-1.5"><div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div><p className="text-[9px] text-green-600 font-bold uppercase tracking-widest">Online</p></div>
            </div>
          </header>
          <main className="px-10 pb-10 max-w-[1600px] w-full">{children}</main>
        </div>
      </div>
