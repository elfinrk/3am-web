...
import { 
  ...
  ArrowRight, CheckCircle, Clock, CreditCard, Copy, QrCode, AlertCircle 
} from "lucide-react";
...
export default function MenuPage() {
  ...
  // --- 2. LOGIKA FILTER ---
  const highlightItem = products.find(p => p.id === 999); // 3AM Coffee Cup
  const foodAndDrinks = products.filter(item => item.category !== "merch"); // Kecuali merch
  ...
  const addToCart = (item: any) => {
    if (item.stock <= 0) {
      setToast({ show: true, msg: "Stok Habis!" });
    ...
    setToast({ show: true, msg: `${item.name} ditambahkan!` });
    setTimeout(() => setToast({ show: false, msg: "" }), 2000);
  };
  ...
  const handleCheckout = () => { if (cart.length === 0) return; setIsCartOpen(false); setShowPayment(true); };
  const confirmPayment = () => { setShowPayment(false); setShowSuccess(true); };
  const closeSuccessModal = () => { setCart([]); setShowSuccess(false); };
  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text);
    setToast({ show: true, msg: "Nomor disalin!" });
    setTimeout(() => setToast({ show: false, msg: "" }), 2000);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9F5E8] font-body text-[#3E2723] overflow-x-hidden relative pb-32">
      ...
      <div className={`fixed top-24 left-1/2 transform -translate-x-1/2 z-[110] transition-all duration-300 ${toast.show ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
        <div className="bg-[#3E2723] text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-3 text-sm font-bold">
      ...
      <main className="container mx-auto px-4 md:px-6 py-8">
          ...
          <section className="mb-12">
            <div className="relative bg-[#2E1C16] rounded-[2rem] p-6 md:p-10 overflow-hidden shadow-2xl flex flex-col md:flex-row items-center gap-8 text-[#F9F5E8]">
              ...
              <div className="w-full md:w-1/2 space-y-4 relative z-10 text-center md:text-left order-1 md:order-2">
                ...
                <p className="text-[#F9F5E8]/80 text-sm">Bawa pulang kopi Anda dengan gaya.</p>
                <div className="pt-2 flex items-center gap-4 justify-center md:justify-start">
                  <span className="font-bold text-2xl">Rp {highlightItem.price.toLocaleString('id-ID')}</span>
                  <button onClick={() => addToCart(highlightItem)} className={`px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg active:scale-95 ${highlightItem.stock > 0 ? 'bg-[#E5DCC5] text-[#3E2723]' : 'bg-gray-500'}`}>
                    {highlightItem.stock > 0 ? "Add +" : "Habis"}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {filteredItems.map((item) => (
            <div key={item.id} className="group bg-white rounded-3xl p-4 border border-[#E5DCC5] hover:border-[#3E2723] transition-all flex flex-col h-full relative overflow-hidden">
              {item.stock <= 0 && <div className="absolute inset-0 bg-black/40 z-10 flex items-center justify-center"><span className="bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">Habis</span></div>}
              <div className="relative w-full aspect-square bg-[#F6F1E7] rounded-2xl mb-3 flex items-center justify-center">
                <Image src={item.img} alt={item.name} fill className="object-contain p-4 group-hover:scale-110 transition-transform" />
                {item.stock > 0 && <button onClick={() => addToCart(item)} className="absolute bottom-2 right-2 w-10 h-10 bg-white rounded-full shadow flex items-center justify-center text-[#3E2723] hover:bg-[#3E2723] hover:text-white transition-all"><Plus size={20} /></button>}
              </div>
              ...
              <div className="mt-auto border-t border-dashed border-[#E5DCC5] pt-2"><span className="font-bold">Rp {item.price.toLocaleString('id-ID')}</span></div>
            </div>
          ))}
        </div>
      </main>

      {/* --- FLOATING BOTTOM BAR (AGAR KERANJANG TERLIHAT) --- */}
      <div className={`fixed bottom-0 left-0 w-full p-4 z-40 transition-transform duration-300 ${totalItems > 0 ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="max-w-4xl mx-auto bg-[#3E2723] text-white rounded-2xl p-4 shadow-2xl flex justify-between items-center cursor-pointer hover:bg-[#2A1D15]" onClick={() => setIsCartOpen(true)}>
             ...
             <div className="flex items-center gap-2 bg-white/20 px-4 py-2 rounded-xl font-bold text-sm">
                Lihat Pesanan <ShoppingBag size={16} />
             </div>
        </div>
      </div>
      ...
      {showPayment && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] p-6 w-full max-w-sm text-center shadow-2xl border border-[#E5DCC5]">
            ...
            <button onClick={confirmPayment} className="w-full py-3 bg-[#3E2723] text-white rounded-xl font-bold">Saya Sudah Bayar</button>
